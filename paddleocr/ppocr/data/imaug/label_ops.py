...
# from tools.infer.predict_system import infer_ocr
...
class VQATokenLabelEncode(object):
    ...
    def __call__(self, data):
        ...
        print("load bbox and label info >>>> ocr_info >>>>>>>>>>>>>>>> ", ocr_info)
    ...
    # this is ocr_info, pls edit modify ocr model
    def _load_ocr_info(self, data):
        if self.infer_mode:
            print("data['image'] >>>>>>>>>> ", data["image"])
            print("shape data['image'] >>>>>>>>>> ", data["image"].shape)
            ...
            text_sys = self.ocr_engine
            dt_boxes, rec_res, _ = text_sys(data["image"])
            ocr_info = [
                {
                    "transcription": rec_res[i][0],
                    "bbox": self.trans_poly_to_bbox(np.array(dt_boxes[i]).tolist()),
